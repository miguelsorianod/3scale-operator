...
func (r *ActiveDocReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	statusReconciler, reconcileStatus, reconcileErr := r.reconcileSpec(activeDocCR, reqLogger)
...
func (r *ActiveDocReconciler) reconcileSpec(activeDocCR *capabilitiesv1beta1.ActiveDoc, logger logr.Logger) (*ActiveDocStatusReconciler, ctrl.Result, error) {
	err := r.validateSpec(activeDocCR)
	if err != nil {
		statusReconciler := NewActiveDocStatusReconciler(r.BaseReconciler, activeDocCR, "", err)
	...
	err = r.checkExternalRefs(activeDocCR, providerAccount, logger)
	if err != nil {
		statusReconciler := NewActiveDocStatusReconciler(r.BaseReconciler, activeDocCR, providerAccount.AdminURLStr, err)
		return statusReconciler, ctrl.Result{}, err
	}
...
func (r *ActiveDocReconciler) checkExternalRefs(resource *capabilitiesv1beta1.ActiveDoc, providerAccount *controllerhelper.ProviderAccount, logger logr.Logger) error {
	errors := field.ErrorList{}

	// Check product referenced by the ActiveDoc Spec is valid
	if resource.Spec.ProductSystemName != nil {
		productList, err := controllerhelper.ProductList(resource.Namespace, r.Client(), providerAccount.AdminURLStr, logger)
		if err != nil {
		...
		if idx < 0 {
			errors = append(errors, field.Invalid(productFldPath, resource.Spec.ProductSystemName, "spec.productSystemName is not a valid reference."))
		}
	}
